...
fetch("lib/sqljs/sql-wasm.wasm")
    ...
    .then(SQL => {
        SQLInstance = SQL;
        const dbName = getURLParam('db');
        return loadDbFromIndexedDB().then(storedData => {
            if (dbName) {
                if (storedData) {
                    const useStored = confirm(
                        `Es wird eine neue Datenbank "${dbName}" geladen.\n\nMöchtest du diese verwenden?\n(Wähle "Abbrechen", um stattdessen die gespeicherte Datenbank aus dem Browser zu laden.)`
                    );
                    if (useStored) {
                        return loadNamedDb(dbName);
                    } else {
                        db = new SQLInstance.Database(new Uint8Array(storedData));
                ...
                if (storedData) {
                    ...
                    loadDefaultDb(); // lädt datenbank1.sqlite
                }
            }
        });
    });
...
function loadNamedDb(dbName) {
    return fetch(`data/${dbName}`)
        .then(response => {
            if (!response.ok) throw new Error(`Datei nicht gefunden: ${dbName}`);
        ...
        .catch(err => {
            ...
            db = new SQLInstance.Database(); // Leere DB
            updateExampleAndSchema();
            clearOutput();
        });
}
